...
void setup() {
    ...
    //init hexagons
    uint16_t sides[] = {1, 2, 3, 28, 29, 30}; 
    palette_idx[0] = random(0,255);
    hexagons[0].initShape(ColorFromPalette( currentPalette, palette_idx[0], BRIGHTNESS, currentBlending),sides,6);
    
    uint16_t sides2[] = {4, 5, 6, 7, 8, 9};
    palette_idx[1] = random(0,255);
    hexagons[1].initShape(ColorFromPalette( currentPalette, palette_idx[1], BRIGHTNESS, currentBlending),sides2,6);
    
    uint16_t sides3[] = {10, 11, 24, 25, 26, 27}; 
    palette_idx[2] = random(0,255);
    hexagons[2].initShape(ColorFromPalette( currentPalette, palette_idx[2], BRIGHTNESS, currentBlending),sides3,6);
    
    uint16_t sides4[] = {12, 13, 14, 15, 16, 17}; 
    palette_idx[3] = random(0,255);
    hexagons[3].initShape(ColorFromPalette( currentPalette, palette_idx[3], BRIGHTNESS, currentBlending),sides4,6);
    
    uint16_t sides5[] = {18, 19, 20, 21, 22, 23}; 
    palette_idx[4] = random(0,255);
    hexagons[4].initShape(ColorFromPalette( currentPalette, palette_idx[4], BRIGHTNESS, currentBlending),sides5,6);
...
void loop()
{
  ...
  else if(mode == HEXAGON)
  {
    for(int i = 0; i < 5; i++)
    {
      ...
      hexagons[i].drawShape(leds, NUM_LEDS, side_len);
     }
  }
